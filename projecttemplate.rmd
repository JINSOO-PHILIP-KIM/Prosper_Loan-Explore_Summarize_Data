...
ggplot(aes(x = StatedMonthlyIncome, y = AmountDelinquent), data = subset(df_copy, !is.na(AmountDelinquent))) +
  geom_point() +
  scale_x_continuous(lim = c(0, quantile(df_copy$StatedMonthlyIncome, 0.99))) +
  scale_y_continuous(lim = c(0, quantile(df_copy$AmountDelinquent, 0.99)))
```

***

This is the 'Top 10 Occupations that borrow the most times' except 'Other' and 'blank'.
```{r echo=FALSE, Univariate_Plots_second}
new <- df_copy %>% 
...
ggplot(aes(x = Occupation, y = how_many), data = occupation_top10_frequent) +
  geom_point() +
  ylab('Count') +
  ggtitle('Top 10 Occupations that borrow the most times')
```



```{r echo=FALSE, Univariate_Plots_third}
...
```{r echo=FALSE, Univariate_Plots_fourth}
ggplot(aes(x = Occupation), data = df) +

```

```{r echo=FALSE, Univariate_Plots_fifth}

```

```{r echo=FALSE, Univariate_Plots_sixth}

```

```{r echo=FALSE, Univariate_Plots_seventh}

```

```{r echo=FALSE, Univariate_Plots_eighth}
