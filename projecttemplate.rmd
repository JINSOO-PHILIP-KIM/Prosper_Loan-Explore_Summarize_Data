...
df_wrangled_5states_PS <- # PS: ProsperScore
  ...
  summarise(profits = 
              sum(Profit * LoanOriginalAmount)) %>% 
  filter(ProsperScore %in% c("8", "6", "7", "4", "5")) 

options(scipen=999)

ggplot(df_wrangled_5states_PS,
       aes(IncomeRange, profits, fill = ProsperScore )) +
  geom_bar(position = "dodge",stat = "identity") +
  ylab("Total profits") +
...
This plot shows that ProsperScore 8 dominates most of the profits.  

```{r, fig.width = 9}
options(scipen=999)
ggplot(subset(df_wrangled_5states_PS, ProsperScore == 8),
       aes(IncomeRange, profits, fill = ProsperScore )) +
  geom_bar(position = "dodge",stat = "identity") +
  ylab("Total profits") +
  ggtitle("Profits by ProsperScore(8) & IncomeRange on 5 states")
...
df_wrangled_5states_PS %>% 
  filter(ProsperScore =="8") %>% 
  summarise(sum1 = sum(profits))

rm(df_wrangled_5states_PS)
```

Target 3 made USD 95,673,093,763 total profits.
...
3. ProsperScore __8 out of 11__ is a good score but cannot estimate the accurate pay back rate.
